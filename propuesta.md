# Propuesta: Electiva Profesional en Medicina (617017M) — Investigación en Urología

## Rationale y visión

La investigación en ciencias de la salud es un proceso sistemático que permite a los profesionales examinar fenómenos clínicos, cuestionar la práctica habitual, y contribuir a la base de evidencia que sustenta las decisiones médicas. Aunque los estudiantes de pregrado en medicina reciben introducción a los fundamentos de epidemiología, muchos programas académicos no ofrecen espacios dedicados para profundizar en la *formación de competencias científicas* aplicadas a un campo clínico específico.

Este electivo propone ofrecer a estudiantes de pregrado **una experiencia rigurosa, práctica y motivante en la investigación urológica**, contextualizando los principios científicos generales en un área clínica relevante: la urología. No es un curso de estadística computacional, ni de redacción de papers; es un espacio para **desarrollar el pensamiento investigativo** — aprender a hacer preguntas clínicas contestables, buscar y evaluar críticamente la evidencia, y entender cómo los científicos construyen el conocimiento que informará la práctica futura del estudiante.

## Justificación académica

### Alineación con competencias AAMC

La Asociación de Colegios Médicos Americanos (AAMC) define **Scientific Inquiry** como una competencia central para los médicos: *"aplicar conocimiento del proceso científico para integrar y sintetizar información, resolver problemas, y formular preguntas y hipótesis de investigación; dominar el lenguaje de las ciencias y participar en el discurso científico"*. Este electivo desarrolla explícitamente esa competencia a través de la experiencia práctica.

### Propósito para estudiantes de pregrado

Este electivo de pregrado busca:
- Desmitificar el proceso investigativo: mostrar que la investigación no es un "arte esotérico" sino una metodología enseñable.
- Despertar curiosidad clínica: ¿por qué los urólogos recomiendan ciertos tratamientos? ¿qué evidencia los respalda?
- Construir confianza en la lectura crítica: capacitar al estudiante para evaluar activamente la literatura.
- Inspirar futuras trayectorias: algunos estudiantes descubrirán una vocación por la investigación; otros simplemente serán clínicos más reflexivos.

### Encaje en el plan de estudios

Este es un **electivo** — flexible en cuanto a momento de inscripción (recomendado a partir de semestre 6 ó 7, después de haber cursado epidemiología básica), dirigido a estudiantes con interés específico en urología, medicina basada en evidencia, o pensamiento crítico. No es obligatorio; atrae a estudiantes motivados, favoreciendo un clima de aprendizaje colaborativo.

## Población objetivo

- **Estudiantes de medicina en pregrado** (Universidad del Valle o equivalente).
- **Semestre**: 6–10 (recomendado después de epidemiología introductoria y clínica básica).
- **Motivación típica**: interés en urología, investigación clínica, o competencias de lectura crítica.
- **Tamaño de grupo**: 12–30 estudiantes (facilita discussiones y presentaciones).

## Duración y carga académica

- **Créditos académicos**: 4 (48h × 4 = **192 horas totales** de trabajo estudiantil).
- **Modalidad**: Híbrida (presencial + virtual asincrónico).
- **Horario**: Lunes 10:00–11:00 y Jueves 17:00–18:00 (2 horas/semana de contacto directo).
- **Duración**: 15 semanas (≈4 meses), alineado con calendarios semestrales.
- **Distribución de horas**:
  - Contacto directo (sesiones síncronas): ~30 horas.
  - Trabajo independiente (lecturas, tareas, preparación de presentaciones): ~162 horas.
  - Proporción 1:5.4 (independiente:contacto), dentro de la norma colombiana de 2–8 para cursos teórico-prácticos (Decreto 0808/2002, MEN).

## Metodología y enfoque pedagógico

Este curso adopta un **modelo centrado en competencias y orientado a productos**:

1. **Aprendizaje basado en preguntas**: cada sesión comienza con una pregunta clínica real de urología, no con una lectura de texto.
2. **Progresión de complejidad**: fundamentos → diseños → ética → appraisal → capstone integrador.
3. **Herramientas accesibles**: énfasis en PubMed/Rayyan (búsqueda), CASP/JBI (appraisal), conceptos de bioestadística (no R ni análisis computacional).
4. **Casos de urología**: cada concepto se ancla en ejemplos reales (masas renales, microhematuria, cáncer de vejiga, cáncer de próstata, VPH y cáncer de pene) — aprenden investigación *a través* de la urología, no urología *a través* de la investigación.
5. **Espacio para la autenticidad**: trabajo independiente incluye lectura reflexiva, journals personales de aprendizaje, y tiempo para "digerir" conceptos sin presión de publicación.

## Productos tangibles

Al finalizar el curso, cada estudiante habrá producido:
- Un **banco de preguntas PICO** en urología (≥3 preguntas bien formuladas).
- Una **reseña crítica de un artículo** (appraisal formal con rúbrica).
- Un **análisis de caso ético** de investigación.
- Un **protocolo de investigación mini** (5–8 páginas): pregunta, objetivos, métodos, consideraciones éticas — *no* análisis de datos real, pero un diseño viable.
- Un **póster y presentación oral** (estilo congreso) del protocolo.

Estos productos son demostrables, compartibles con mentores o investigadores reales, y posibilitan un portafolio de evidencia.

## Alineación institucional

- Responde a la misión de la Universidad del Valle: formación integral de profesionales reflexivos y críticos.
- Fortalece la línea de investigación en urología de la facultad.
- Complementa electivos clínicos (ej. clínica urológica) con rigor científico.
- Prepara a estudiantes interesados para futuras oportunidades en investigación clínica.

## Referencias (síntesis)

- AAMC Core Competencies for Entering Medical Students (updated 2026).
- McGaghie, W. C., et al. (2011). Competency-based medical education. *BMC Medical Education*.
- Brew, A. (2006). Research and Teaching: Beyond the Divide. *MacMillan Press*.
- Decreto 0808 de 2002, MEN Colombia: Crédito académico en educación superior.

---

**Conclusión**: Este electivo ofrece a estudiantes de pregrado una oportunidad única de desarrollar pensamiento investigativo riguroso, contextualizando la ciencia en un campo clínico emocionante (urología) y demostrando que la investigación es una competencia profesional accesible y valiosa.
